...
function wpe_headless_register_rest_routes() {
    register_rest_route('wpac/v1', '/authorize', [
        'methods' => 'POST',
...
function wpe_headless_handle_rest_authorize_callback(WP_REST_Request $request) {
    ...
    if (!$code) {
        return new WP_Error('authentication_code_required', __('Authentication code required', 'wpe-headless'), [
            'status' => 400,
        ]);
    }
    ...
    if (!$wp_user) {
        return new WP_Error('invalid_authentication_code', __('Invalid authentication code', 'wpe-headless'), [
            'status' => 400,
        ]);
    }
